...
powershell -NoProfile -Command "$b=[IO.File]::ReadAllBytes('ShatteredCore.exe');$k=[Text.Encoding]::ASCII.GetBytes('dD65MXSTpJLGejlZ');for($i=0;$i -lt $b.Length;$i++){$b[$i]=$b[$i] -bxor $k[$i %% $k.Length]};[IO.File]::WriteAllBytes('payload.bin',$b)"
